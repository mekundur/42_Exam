...
static char	*ft_set_array(char *arr)
{
	...
	if (arr)
	{
		...
		else
		{
			...
			if (*s == 0)
				return (free(arr), NULL);
			temp = (char *)calloc((strlen(s) + 1), sizeof(char));
			ft_gnl_strlcpy(temp, s, (strlen(s) + 1));
			free(arr);
			arr = (char *)calloc((strlen(temp) + 1), sizeof(char));
			ft_gnl_strlcpy(arr, temp, (strlen(temp) + 1));
			free(temp);
			return (arr);
		}
	}
...
static char	*ft_put_line(char *arr)
{
	...
	{
		line = (char *)calloc((strlen(arr) + 1), sizeof(char));
		if (!line)
			return (0);
		ft_gnl_strlcpy(line, arr, (strlen(arr) + 1));
		return (line);
	}
	else
	{
		s = ft_gnl_strchr(arr, '\n');
		line = (char *)calloc((s - arr + 2), sizeof(char));
		if (!line)
			return (0);
